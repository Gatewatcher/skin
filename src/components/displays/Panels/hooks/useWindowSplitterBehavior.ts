...
import type { CommittedValues, PanelDataMap, ResizeHandler } from '../types';
import {
  adjustByDelta,
  getFlexGrow,
  getPanel,
  ...
  useEffect(() => {
    ...
    const cleanupFunctions = handles.map(handle => {
      ...
      const onKeyDown = (event: KeyboardEvent) => {
        if (!panelSizeBeforeCollapse.current) {
          return;
        }
        switch (event.key) {
          case 'Enter': {
    ...
    return () => {
      cleanupFunctions.forEach(
        cleanupFunction => cleanupFunction && cleanupFunction(),
      );
    };
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      switch (event.key) {
        ...
        case 'F6': {
          ...
          const index = getResizeHandleIndex(handleId) ?? 0;
