...
describe('EllipsisDataModal', () => {
  const TEST_ID: TestId = 'ellipsis-data-modal';
  ...
  const renderComponent = ({
    ...
    data = DEFAULT_DATA,
    ...props
  }: Partial<EllipsisDataModalProps<User>> = {}) =>
    render(
      <EllipsisDataModal data={data} data-testid={TEST_ID} {...props}>
        {children}
      </EllipsisDataModal>,
    );
  ...
  it('should render', async () => {
    renderComponent();
    await expectToBeVisibleInTheDocument('ellipsis-data-modal');
  });
  ...
  it('should have badge count', async () => {
    renderComponent({
      moreText: count => <EllipsisDataModal.BadgeCount count={count} />,
    });
  ...
  it('should have modal title', async () => {
    ...
    await openEllipsedContent();
    await expectToBeVisibleInTheDocument('Modal title', screen.findByText);
  });
});
