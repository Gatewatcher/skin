...
import type { LocaleLoader } from './i18n/localeLoader';
...
export class ExpressionDescriptor {
      ...
      description = this.getSegmentDescription(
        ...
        (s, form) => {
          ...
          if (this.options.tzOffset) {
            ...
            const hour: number =
              ...
              (this.options.tzOffset ? this.options.tzOffset : 0);
            if (hour >= 24) {
              parsedExp++;
            } else if (hour < 0) {
              parsedExp--;
          ...
          if (s.indexOf('#') > -1) {
            ...
            const dayOfWeekOfMonthNumber: string = s.substring(
              s.indexOf('#') + 1,
            );
        ...
        s => {
          if (parseInt(s) == 1) {
            ...
            return StringUtilities.format(
              this.i18n.commaEveryX0DaysOfTheWeek(s),
              s,
            );
          }
        },
        ...
        s => {
          ...
          if (s.indexOf('#') > -1) {
            ...
            format =
              this.i18n.commaOnThe(dayOfWeekOfMonthNumber).trim() +
            ...
            const domSpecified = this.expressionParts[3] != '*';
            format = domSpecified
              ? this.i18n.commaAndOnX0()
              : this.i18n.commaOnlyOnX0(s);
          }
  ...
  protected getMonthDescription() {
    ...
    const description: string | null = this.getSegmentDescription(
      ...
      () => {
        return this.i18n.commaOnlyInMonthX0
          ? this.i18n.commaOnlyInMonthX0()
  ...
  protected getSegmentDescription(
    ...
    if (!expression) {
      ...
      if (segments[0].indexOf('-') > -1) {
        ...
        const rangeSegmentDescription: string =
  ...
  protected generateRangeSegmentDescription(
    ...
    const rangeSegment1Description: string = getSingleItemDescription(
      rangeSegments[0],
      1,
    );
    const rangeSegment2Description: string = getSingleItemDescription(
      rangeSegments[1],
      2,
    );
