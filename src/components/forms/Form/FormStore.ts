...
export class FormStore {
   ...
   * This needs to be refilled with initialValues instead of store value.
   */
  ...
  private setInitialValues = (initialValues: Store, init: boolean) => {
    ...
    if (init) {
      ...
      // When the field is not `preserve`, we need to fill this with initialValues instead of store.
  ...
  // Param pure only return field which has a `name`.
  // Default: false
  ...
  private getFieldsValue = (
    ...
    fieldEntities.forEach((entity: FieldEntity | InvalidateFieldEntity) => {
      ...
      // since parent field is already taken into account.
  ...
  private getFieldsError = (nameList?: NamePath[]) => {
    ...
    return fieldEntities.map((entity, index) => {
      ...
      return {
        name: getNamePath(nameList ? nameList[index] : null),
        errors: [],
        warnings: [],
      };
    });
  ...
  private isFieldsTouched = (...args: unknown[]) => {
    this.warningUnhooked();
    ...
    if (args.length === 0) {
      ...
      namePathList = (arg0 as number[] | string[]).map(getNamePath);
    ...
    // Generate a nest tree for validating.
    ...
    fieldEntities.forEach(field => {
      ...
      // Find matched entity and put it into a list.
      (namePathList || []).forEach(shortNamePath => {
        if (
  ...
  private resetWithFieldInitialValue = (
    ...
    // Create cache
    const cache: NameMap<Set<{ entity: FieldEntity; value: unknown }>> =
      new NameMap();
    ...
    fieldEntities.forEach(field => {
      ...
      // Record only if it has `initialValue`
  ...
  private getFields = (): InternalFieldData[] => {
    ...
    return entities.map((field: FieldEntity): InternalFieldData => {
      const namePath = field.getNamePath();
  ...
  private setError = (
    ...
    this.setFields([
      { name: [prefix, name].filter(Boolean) as NamePath, errors: error },
    ]);
  };
  ...
  private setErrors = (
    ...
    this.setFields(
      Object.entries(errors).map(([name, error]) => ({
        name: [prefix, name].filter(Boolean) as NamePath,
        errors: error,
      })),
    );
  };
    ...
    return (
      ...
      if (
        ...
        if (
          ...
          // Notify that field is unmounted
   ...
   * We need delay validation in case Field is under render props
   */
  ...
  private setFieldValue = (name: NamePath, value: unknown) => {
    this.setFields([
      {
        name,
        value,
  ...
  private validateFields: InternalValidateFields = (
    ...
    // Collect result in a promise list
    ...
    this.getFieldEntities(true).forEach((field: FieldEntity) => {
      ...
      if (!field.getMeta().rules.length && !field.getErrors().length) {
        return;
      }

      const fieldNamePath = field.getNamePath();
      // Add field validate rule into a promise list
